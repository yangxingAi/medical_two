...
import com.cqupt.software_1.Util.CDividedQuickRduct;
import com.cqupt.software_1.Util.Connect;
import com.cqupt.software_1.Util.FieldStats;
import com.cqupt.software_1.dto.DownloadDTO;
...
//    }
    ...
    /**
     *   收到特征划分段数和值  保存在新表   这部分在python做了
     *
     */


    @PostMapping("/featureCreate")
    public R<List<Map<String,Integer>>> featureCreate(@RequestBody List<FeatureCreateDTO> featureCreateDTO){



        List<Map<String,Integer>> res = cardioTrainService.splitFiled(featureCreateDTO);
    ...
    @PostMapping("peopleSplit")
    public R peopleSplit(@RequestParam String tableName,@RequestParam int numOfSamples, @RequestParam float sicknessRate,@RequestParam int pageNum){

        Set<Integer> satisfiedSamples = new HashSet<>();
        ...
        try {
            ...
            CDividedQuickRduct sorter = new CDividedQuickRduct();
            // 连接数据库并查询数据
            int[][] table = con.fetchDataFromDatabase(tableName);

            // 设置属性数量（除去标签列）
            int numAttributes = table[0].length - 1;

            // 进行数据分层划分
            sorter.twoDimensionQuickSort(table, numAttributes,numOfSamples,sicknessRate);

            // 输出划分结果

            satisfiedSamples =sorter.getSatisfiedSamples();

            PageHelper.startPage(pageNum,15);
            // 查询出id对应的原表结果
            List<CardioTrain> peopleRes = (List<CardioTrain>) cardioTrainService.listByIds(satisfiedSamples);

            map = sorter.printSetInfo(satisfiedSamples);

            map.put("原表结果",peopleRes);

            map.put("id集合",satisfiedSamples);

            PageInfo pageInfo = new PageInfo<>(peopleRes);
        ...
        return new R<>(200,"成功",map,pages);
    }




    @PostMapping(value = "downloadData" ,produces = "text/csv;charset=ISO-8859-1")
    public ResponseEntity<FileSystemResource> downloadData(@RequestBody DownloadDTO downloadDTO) {

        Set<Integer> satisfiedSamples = downloadDTO.getSatisfiedSamples();
        String tableName = downloadDTO.getTableName();

        List<Map<String,Object>> res =  dataService.selectListById(satisfiedSamples,tableName);

        String filePath = null;
        // 保存到 CSV 文件
        try {
           filePath = "src/main/resources/files/result.csv";
            CSVWriter csvWriter = new CSVWriter(new FileWriter(filePath));

            // 写入 CSV 文件头部（列名）
            if (!res.isEmpty()) {
                String[] header = res.get(0).keySet().toArray(new String[0]);
                csvWriter.writeNext(header);
            }

            // 写入查询结果数据
            for (Map<String, Object> row : res) {
                String[] rowValues = row.values().stream()
                        .map(Object::toString)
                        .toArray(String[]::new);
